...
- Provide a worker backfill target for YouTube feeds that takes a feed id, uses the `yt-dlp` CLI to extract `/videos` and `/shorts`, supports cookies and explicit JavaScript runtime configuration, skips subscriber/member-only videos, and stores full yt-dlp metadata in item `raw_extension_data`.
